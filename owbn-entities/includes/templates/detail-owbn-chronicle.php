...
if ( ! empty( $slug ) && function_exists( 'owc_get_chronicle_detail' ) ) {
    ...
    if ( ! is_wp_error( $data ) && ! empty( $data ) && function_exists( 'owc_render_chronicle_detail' ) ) {
        ...
        echo '<div id="content" class="owc-template-wrapper"><p>' . esc_html__( 'Chronicle not found.', 'owbn-entities' ) . '</p></div>';
    }
} else {
    echo '<div id="content" class="owc-template-wrapper"><p>' . esc_html__( 'No chronicle specified.', 'owbn-entities' ) . '</p></div>';
}
