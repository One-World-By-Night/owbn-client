...
if ( ! empty( $slug ) && function_exists( 'owc_get_coordinator_detail' ) ) {
    ...
    if ( ! is_wp_error( $data ) && ! empty( $data ) && function_exists( 'owc_render_coordinator_detail' ) ) {
        ...
        echo '<div class="owc-template-wrapper"><p>' . esc_html__( 'Coordinator not found.', 'owbn-entities' ) . '</p></div>';
    }
} else {
    echo '<div class="owc-template-wrapper"><p>' . esc_html__( 'No coordinator specified.', 'owbn-entities' ) . '</p></div>';
}
