...
class OWC_Coordinator_Votes_Section_Widget extends Widget_Base {

    public function get_name() { return 'owc_coordinator_votes_section'; }
    public function get_title() { return __( 'Coordinator Vote History', 'owbn-entities' ); }
    ...
    protected function render() {
        ...
        if ( is_wp_error( $data ) || empty( $data ) ) {
            if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                echo '<p style="color:#999;text-align:center;">[Coordinator Vote History — select a coordinator or view on frontend]</p>';
            }
            return;
        }

        if ( function_exists( 'owc_render_entity_vote_history' ) ) {
            echo owc_render_entity_vote_history( 'coordinator', $data['slug'] ?? '' );
        }
    }
}
