...
class OWC_Chronicle_About_Section_Widget extends Widget_Base {

    public function get_name() { return 'owc_chronicle_about_section'; }
    public function get_title() { return __( 'Chronicle About', 'owbn-entities' ); }
    ...
    protected function render() {
        ...
        if ( is_wp_error( $data ) || empty( $data ) ) {
            if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                echo '<p style="color:#999;text-align:center;">[Chronicle About — select a chronicle or view on frontend]</p>';
            }
            return;
        }

        if ( function_exists( 'owc_render_chronicle_about' ) ) {
            echo owc_render_chronicle_about( $data );
        }
    }
}
