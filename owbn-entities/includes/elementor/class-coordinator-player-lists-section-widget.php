...
class OWC_Coordinator_Player_Lists_Section_Widget extends Widget_Base {

    public function get_name() { return 'owc_coordinator_player_lists_section'; }
    public function get_title() { return __( 'Coordinator Player Lists', 'owbn-entities' ); }
    ...
    protected function render() {
        ...
        if ( is_wp_error( $data ) || empty( $data ) ) {
            if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                echo '<p style="color:#999;text-align:center;">[Coordinator Player Lists — select a coordinator or view on frontend]</p>';
            }
            return;
        }

        if ( function_exists( 'owc_render_coordinator_player_lists' ) ) {
            echo owc_render_coordinator_player_lists( $data );
        }
    }
}
