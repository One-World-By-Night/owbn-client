...
class OWC_Coordinator_Documents_Section_Widget extends Widget_Base {

    public function get_name() { return 'owc_coordinator_documents_section'; }
    public function get_title() { return __( 'Coordinator Documents', 'owbn-entities' ); }
    ...
    protected function render() {
        ...
        if ( is_wp_error( $data ) || empty( $data ) ) {
            if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                echo '<p style="color:#999;text-align:center;">[Coordinator Documents — select a coordinator or view on frontend]</p>';
            }
            return;
        }

        if ( function_exists( 'owc_render_coordinator_documents' ) ) {
            echo owc_render_coordinator_documents( $data );
        }
    }
}
