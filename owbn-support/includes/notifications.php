...
function owbn_support_notify_ticket_created( $ticket_id, $data ) {
    ...
    wp_mail( $agent->user_email, $subject, $body, owbn_support_mail_headers() );
}
...
function owbn_support_notify_reply( $reply_id, $data ) {
    ...
    if ( $is_agent_reply && $author && $author->user_email ) {
        ...
        wp_mail( $author->user_email, $subject, $body, owbn_support_mail_headers() );
 ...
 * Common mail headers.
 */
function owbn_support_mail_headers() {
    $from_name  = get_option( 'blogname', 'OWBN Support' );
    $from_email = get_option( 'admin_email', 'web@owbn.net' );
    return array(
