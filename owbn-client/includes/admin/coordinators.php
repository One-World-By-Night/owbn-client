...
/**
 * OWBN-Client Coordinators Dashboard
 *
 * Shows data source, cache status, record count, and gateway registration
 * for the Coordinators data type. Provides cache clear/refresh actions.
 *
 * @package OWBN-Client
 */
...
function owc_render_coordinators_page()
{
    ...
    // ── Status data ────────────────────────────────────────────────────────────
    $is_local    = owc_manager_active() && owc_coordinators_enabled();
    $remote_base = owc_get_remote_base();
    $cache       = get_transient('owc_coordinators_cache');
    ...
    foreach ($gw_sources as $source) {
        if (isset($source['types']) && in_array('coordinator', $source['types'], true)) {
            $gw_has_type = true;
            break;
        }
    }
