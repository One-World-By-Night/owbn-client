...
function owc_render_territories_page()
{
    ...
    if (isset($_POST['owc_clear_territories_cache']) && check_admin_referer('owc_territories_cache_action')) {
        ...
        add_settings_error('owc_territories', 'cache_cleared', __('Territories cache cleared.', 'owbn-core'), 'success');
    }
    ...
    if (isset($_POST['owc_refresh_territories_cache']) && check_admin_referer('owc_territories_cache_action')) {
        ...
        if (is_wp_error($result)) {
            ...
            add_settings_error('owc_territories', 'cache_refreshed', __('Territories cache refreshed.', 'owbn-core'), 'success');
        }
    }
    ...
    if ($is_local) {
        $source_label = __('Local (Territory Manager active)', 'owbn-core');
    } elseif ($remote_base) {
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('Territories', 'owbn-core'); ?></h1>
                ...
                <tr>
                    ...
                    <td><?php
                        if ($is_local) {
                            ...
                            echo absint($cpt_count) . ' ' . esc_html__('published', 'owbn-core');
                            if (is_array($cache)) {
                                echo ' &mdash; ' . absint(count($cache)) . ' ' . esc_html__('cached', 'owbn-core');
                            }
                        } elseif (is_array($cache)) {
                            echo absint(count($cache)) . ' ' . esc_html__('territories cached', 'owbn-core');
                        } else {
                ...
                <tr>
                    ...
                    <td><?php
                        if ($gw_enabled && $gw_has_type) {
                            ...
                            esc_html_e('Enabled — no territory source registered', 'owbn-core');
                        } else {
        ...
        <form method="post" style="display:inline-block; margin-right:8px;">
            ...
            <?php submit_button(__('Clear Cache', 'owbn-core'), 'secondary', 'owc_clear_territories_cache', false); ?>
        </form>
        <form method="post" style="display:inline-block;">
            <?php wp_nonce_field('owc_territories_cache_action'); ?>
            <?php submit_button(__('Refresh Cache', 'owbn-core'), 'secondary', 'owc_refresh_territories_cache', false); ?>
        </form>
    </div>
    <?php
}
